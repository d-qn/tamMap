...
##' @param dirGeo, a string of length 1 the directory in the package inst/extdata/shp/ where to look for geo spatial data. Currently CH or World
...
shp_path <- function(y = 2018, features = c('municipalities', 'cantons', 'lakes', 'country'), dirGeo = 'CH') {
  ...
  if(dirGeo == 'CH') {
    ...
    structure(files, name = dirGeo)
  } else {
    stop("\n The dirGeo provided ", dirGeo, " unknown!")
  }
}